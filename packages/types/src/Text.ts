...
export default class Text extends String implements Codec {
  ...
  constructor (value: Text | string | AnyU8a = '') {
    ...
    this.raw = this; // FIXME Remove this when we remove Base
  }

  static decodeText (input: any): string | Text {
    if (isString(input) || input instanceof Text) {
      return input;
    ...
    } else if (input && isFunction(input.toString)) {
